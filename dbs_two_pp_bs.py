...
def resample_two(Xp,Xn,n=5):
	Xpp['PD'] = 1
	Xet['PD'] = 0
	X = pd.concat([Xpp,Xet],axis=0,ignore_index=True)
	cols = ['PD','One','Two']
	X = X[cols]
	i = 1
	XS=X
...
def GLM_two(XS,n=2000):
	...
	plt.savefig('Trace_PPET_Bootstrap.png')
...
def plot_two(X, One, Two, intercept):
	...
	plt.title('Classification Of ET And PP With Bootstrap Samples', fontsize=16)
	plt.tight_layout()
	plt.savefig('PD_PPET.png')
	#plt.show()
	plt.traceplot(trace)
	plt.savefig('ConvergencePE.png')
	#plt.show()
	pps=ps
	ps[ps<m] = int(0)
	ps[ps>=m] = int(1)
	...
	cm.to_csv('Confusion Matrix PPET', index=False)
	return ps, pps, cm
...
def RFC_two(XS):
	...
	cm = cpd[['Predicted','Actual_Negative','Actual_Positive']]
	cm.to_csv('Confusion Matrix PPET', index=False)
	return cm, rfc_mse, cpdsplit, rfc_mse_split
